...
OUTPUT_DIR = Path(__file__).parent.parent.parent.parent / 'data' / 'climate_models' / 'temp_prediction' / '1_seasonal_means'
...
def save_gcm_seasonal_means(hist_means, fut_means, model_name, variable):
    ...
    hist_file = OUTPUT_DIR / f'seasonal_means_historical_{model_name}_{variable}_ssp370.csv'
    hist_means.to_csv(hist_file)
    print(f"\nSaved GCM historical seasonal means to: {hist_file}")
    
    # Save future means
    fut_file = OUTPUT_DIR / f'seasonal_means_future_{model_name}_{variable}_ssp370.csv'
