...
HIST_OUTPUT_DIR   = Path(__file__).parent.parent.parent / 'data' / 'precipitation' / 'hist_GCM' / 'csv'
FUTURE_OUTPUT_DIR = Path(__file__).parent.parent.parent / 'data' / 'precipitation' / 'future_GCM' / 'csv'
...
def save_gcm_seasonal_means(hist_means, fut_means, model_name):
    ...
    hist_file = HIST_OUTPUT_DIR / f'seasonal_means_historical_{model_name}_pr_ssp370.csv'
    hist_means.to_csv(hist_file)
    print(f"\nSaved GCM historical seasonal means to: {hist_file}")
    
    # Save future means
    fut_file = FUTURE_OUTPUT_DIR / f'seasonal_means_future_{model_name}_pr_ssp370.csv'
