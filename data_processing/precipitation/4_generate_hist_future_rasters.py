#!/usr/bin/env python3
"""
Step 4: Generate historical and future GCM precipitation rasters at 30m
...
Output locations:
  data/precipitation/hist_GCM/  – one TIFF per model × season (1990-2019)
  data/precipitation/future_GCM/ – one TIFF per model × season (2035-2064)

Filename convention:
  precip_hist_{MODEL}_{SEASON}_1990-2019_30m.tif
  precip_future_{MODEL}_{SEASON}_2035-2064_30m.tif
...
HIST_CSV_DIR   = REPO_ROOT / 'data' / 'precipitation' / 'hist_GCM' / 'csv'
FUTURE_CSV_DIR = REPO_ROOT / 'data' / 'precipitation' / 'future_GCM' / 'csv'
HIST_OUT_DIR   = REPO_ROOT / 'data' / 'precipitation' / 'hist_GCM'
FUTURE_OUT_DIR = REPO_ROOT / 'data' / 'precipitation' / 'future_GCM'

# Spatial template: any GRIDMET 30m seasonal raster gives us the target grid
TEMPLATE_RASTER = REPO_ROOT / 'data' / 'precipitation' / 'obs_GRIDMET' / 'processed' / 'seasonal' / 'precip_final_30m_2006-2020_djf.tif'
...
def load_seasonal_means_csv(csv_path):
    ...
    Returns a dict mapping season (upper-case) to a numpy array of the
    6 grid-cell values in column order.
    """
    df = pd.read_csv(csv_path, index_col=0)
    df.columns = [c.strip().strip('"') for c in df.columns]
    # build dict: season -> array shape (n_points,)
    return {season: df.loc[season].values.astype(float)
...
def parse_coords(csv_path):
    ...
    return np.array(coords)          # shape (n_points, 2), col order: [lon, lat]
...
def main():
    ...
    # Use the first available CSV to extract coordinates (same for all models)
    first_csv = HIST_CSV_DIR / f'seasonal_means_historical_{MODELS[0]}_pr_ssp370.csv'
    coords_lonlat = parse_coords(first_csv)
    print(f"GCM grid points: {len(coords_lonlat)} points\n")

    for model in MODELS:
        print(f"Processing: {model}")

        hist_csv   = HIST_CSV_DIR   / f'seasonal_means_historical_{model}_pr_ssp370.csv'
        future_csv = FUTURE_CSV_DIR / f'seasonal_means_future_{model}_pr_ssp370.csv'
        ...
        for season in SEASONS:
            ...
            save_raster(hist_grid, profile, HIST_OUT_DIR /
                        f'precip_hist_{model}_{season_lc}_{HIST_PERIOD}_30m.tif', nodata)

            # Future
            future_grid = idw_interpolate(coords_lonlat, future_means[season],
                                          shape, transform, crs)
            future_grid[~valid_mask] = nodata
            save_raster(future_grid, profile, FUTURE_OUT_DIR /
                        f'precip_future_{model}_{season_lc}_{FUTURE_PERIOD}_30m.tif', nodata)
