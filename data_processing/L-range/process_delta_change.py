...
Processes all GCM model directories found directly under data/L-range/.
...
# ---------------------------------------------------------------------------
# Resolve paths — this script lives in data_processing/L-range/
# ---------------------------------------------------------------------------
...
GCM_DIR    = DATA_DIR   # model dirs live directly under L-range/
OBS_DIR    = os.path.join(DATA_DIR, "MODIS_baseline_obs")
MASK_PATH  = os.path.join(DATA_DIR, "watershed-mask", "watershed_mask.asc")
HIST_TIFF_DIRS = {
    "et":  os.path.join(REPO_ROOT, "data", "ET", "hist_L-range"),
    "lai": os.path.join(REPO_ROOT, "data", "LAI", "hist_L-range"),
}
FUTURE_TIFF_DIRS = {
    "et":  os.path.join(REPO_ROOT, "data", "ET", "future_L-range"),
    "lai": os.path.join(REPO_ROOT, "data", "LAI", "future_L-range"),
}
OBS_TIFF_DIRS = {
    "et":  os.path.join(REPO_ROOT, "data", "ET", "obs_MODIS"),
    "lai": os.path.join(REPO_ROOT, "data", "LAI", "obs_MODIS"),
}
CF_TIFF_DIRS = {
    "et":  os.path.join(REPO_ROOT, "data", "ET", "change_factors"),
    "lai": os.path.join(REPO_ROOT, "data", "LAI", "change_factors"),
}
PROJ_TIFF_DIRS = {
    "et":  os.path.join(REPO_ROOT, "data", "ET", "proj"),
    "lai": os.path.join(REPO_ROOT, "data", "LAI", "proj"),
}
...
def main():
    ...
    for var in variables:
        ...
        for season in SEASONS:
            ...
            write_tiff(
                os.path.join(OBS_TIFF_DIRS[var], f"{var_upper}_{season}_obs_baseline_MODIS_{OBS_YEARS[0]}-{OBS_YEARS[-1]}.tiff"),
                ref_header, obs_mean,
            )
    ...
    for model_name in model_names:
        ...
        for var in variables:
            ...
            for season in SEASONS:
                ...
                write_tiff_resampled(
                    os.path.join(HIST_TIFF_DIRS[var],
                                 f"{var_upper}_{model_name}_{season}_hist_L-range_{HIST_YEARS[0]}-{HIST_YEARS[-1]}_30m.tiff"),
                ...
                write_tiff_resampled(
                    os.path.join(FUTURE_TIFF_DIRS[var],
                                 f"{var_upper}_{model_name}_{season}_future_L-range_{FUTURE_YEARS[0]}-{FUTURE_YEARS[-1]}_30m.tiff"),
                ...
                write_tiff(
                    os.path.join(CF_TIFF_DIRS[var], f"{var_upper}_{model_name}_{season}_change_factor.tiff"),
                    model_header, cf,
                )
                ...
                write_tiff(
                    os.path.join(PROJ_TIFF_DIRS[var], f"{var_upper}_{model_name}_{season}_downscaled_future.tiff"),
    ...
    for var in variables:
        write_results_analysis(all_model_stats, var, PROJ_TIFF_DIRS[var])
    ...
    print(f"{'=' * 60}")
    for var in variables:
        print(f"  {var.upper()} hist_L-range   : {os.path.relpath(HIST_TIFF_DIRS[var], REPO_ROOT)}/")
        print(f"  {var.upper()} future_L-range : {os.path.relpath(FUTURE_TIFF_DIRS[var], REPO_ROOT)}/")
        print(f"  {var.upper()} obs_MODIS      : {os.path.relpath(OBS_TIFF_DIRS[var], REPO_ROOT)}/")
        print(f"  {var.upper()} change_factors : {os.path.relpath(CF_TIFF_DIRS[var], REPO_ROOT)}/")
        print(f"  {var.upper()} proj           : {os.path.relpath(PROJ_TIFF_DIRS[var], REPO_ROOT)}/")
