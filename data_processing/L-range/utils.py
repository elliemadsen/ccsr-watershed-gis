...
def write_tiff_resampled(filepath, header, data, target_res=30, crs="EPSG:26918",
                         target_crs=None):
    """
    Write a GeoTIFF from an ASC header + 1-D data array, resampled to target_res metres.

    Optionally reprojects to a different CRS (e.g. EPSG:32618) in the same step.
    Uses rasterio bilinear resampling throughout.
    ...
    header : dict  (ncols, nrows, xllcorner, yllcorner, cellsize, nodata_value)
    data : np.ndarray  (1-D, length = ncols * nrows)
    target_res : int   Target cell size in metres (default 30 m)
    crs : str   Source EPSG code of the input data (default: EPSG:26918)
    target_crs : str or None
        Output CRS.  If None or same as crs, no reprojection is performed.
        Pass e.g. "EPSG:32618" to reproject while resampling.
    """
    import rasterio
    ...
    os.makedirs(os.path.dirname(filepath), exist_ok=True)
    ncols_src = int(header["ncols"])
    nrows_src = int(header["nrows"])
    cellsize = float(header["cellsize"])
    west = float(header["xllcorner"])
    north = float(header["yllcorner"]) + nrows_src * cellsize
    nodata = float(header["nodata_value"])

    src_crs_obj = CRS.from_string(crs)
    src_transform = from_origin(west, north, cellsize, cellsize)
    grid = data.reshape(nrows_src, ncols_src).astype(np.float32)

    dst_crs_obj = CRS.from_string(target_crs) if target_crs else src_crs_obj

    # Use calculate_default_transform when changing CRS, otherwise simple scale.
    if target_crs and CRS.from_string(target_crs) != src_crs_obj:
        dst_transform, ncols_dst, nrows_dst = rasterio.warp.calculate_default_transform(
            src_crs_obj, dst_crs_obj,
            ncols_src, nrows_src,
            left=west, bottom=float(header["yllcorner"]),
            right=west + ncols_src * cellsize,
            top=north,
            resolution=target_res,
        )
    else:
        scale = cellsize / target_res
        nrows_dst = round(nrows_src * scale)
        ncols_dst = round(ncols_src * scale)
        dst_transform = from_origin(west, north, target_res, target_res)

    dst_data = np.empty((nrows_dst, ncols_dst), dtype=np.float32)
